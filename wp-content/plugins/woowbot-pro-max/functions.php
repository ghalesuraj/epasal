...
function qcld_wb_chatbot_product_details(){
    ...
    if ($product->is_in_stock()) {
        if ($product_type == "variable") {
            ...
            if(get_option('wp_chatbot_hide_product_details_add_to_cart') != 1){
...
function qcld_wpbot_get_order_by_username($user_name){
    ...
    if ($response['order_num'] > 0) {
        ...
        $response['message'] .= qcld_wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_sorry'))) . '! <strong>' . ($user_name) . '</strong>';
        $order_html .= '<p>' . qcld_wpb_randmom_message_handle(maybe_unserialize(get_option('qlcd_wp_chatbot_order_not_found'))) . '</p>';
    }
    $response['html'] = $order_html;
    return $response;
...
add_action('init', 'get_page_by_title_two');
if ( ! function_exists( 'get_page_by_title_two' ) ){
    function get_page_by_title_two( $page_title) {
        global $wpdb;
        $post_type = 'page';
