...
$(document).on('click','.wp-chatbot-support-cross',function () {

    var langItems = $(this).parent().parent().parent().children('.row');
    $( "#draggable" ).draggable($(this).parent().parent());

    $( ".qcfaq-inner" ).sortable({
...
(function($) {
    ...
    if(flieElementOpenAi){
        ...
        $(".file_form").on('change','#openfileinput',function(){
            ...
            $('.success-message').html('File is Processing');
        ...
        $("#openaiFTListGPT").on('click','.remove_ft_modelgpt305_openai',function(){
            $(this).hide();
            var ft_id = $(this).attr('data-id');
            console.log(ft_id)
            $.ajax({
                ...
                dataType: "JSON",
                data:  {
                    action : 'openai_ft_model_gpt_delete',
                    nonce:ajax_object.ajax_nonce,
                    ft_id: ft_id
                },
                success: function (response) {
            ...
            success: function(res) {
                if((res.data != undefined)){
                    ...
                    function itemfunction(item) {
                        text += '<tr><td>'+item.filename +'</td><td>' +item.id+'</td><td><a class="btn-success btn floated create_ft_model_modal" data-id="'+item.id+'">Create FT Model</a> <a data-id="'+item.id+'" class="btn-success btn floated download_ft_file">Download</a> <a class="btn-danger btn floated remove_file_openai" data-id="'+item.id+'">Remove</a></td></tr>';
                    }
                    document.getElementById("openaiFileList").innerHTML = text;
                }
            }
        });
        $('#openaiFileList').on('click','.download_ft_file',function(){
            ...
            $.ajax({
                ...
                dataType: "JSON",
                data:  {
                    action : 'qcld_openai_file_dowload',
                    nonce: ajax_object.ajax_nonce,
                    file_id: f_id
                },
                success: function (response) {
                    var fileName = response[0].fileinfo.filename;
                    var data = response[0].filedata;
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                data.forEach(itemfunction);
                function itemfunction(item) {
                    text += '<tr><td>' +item[0]+'</td><td>'+item[1]+'</td><td>'+item[2]+'</td><td> <a class="btn-danger btn floated remove_ft_model_openai" data-id="'+item[0]+'">Remove</a></td> </tr>';
                }
        ...
        $.ajax({
            ...
            dataType: "JSON",
            data: {
                action : 'openai_finetune_gpt3_list',
                nonce: ajax_object.ajax_nonce,
            },
            success: function(res) {
                data = res;
                ...
                function itemfunction(item) {
                    console.log(item)
                    text += '<tr><td>' +item[0]+'</td><td>'+item[1]+'</td><td>'+item[2]+'</td><td> <a class="btn-danger btn floated remove_ft_modelgpt305_openai" data-id="'+item[0]+'">Remove</a></td></tr>';
                }
                document.getElementById("openaiFTListGPT").innerHTML = text;
    ...
    var flieElementOpenAi_gpt = document.getElementById("openfileinput_gpt");

    if(flieElementOpenAi_gpt){
        $(".file_form_gpt").on('change','#openfileinput_gpt',function(){
            
            var file_data = $('#openfileinput_gpt').prop('files')[0];
            var form_data = new FormData();
            form_data.append('file', file_data);
            form_data.append('action', 'openai_gpt_file_upload');
            form_data.append('nonce',ajax_object.ajax_nonce);
            form_data.append('purpose','fine-tune');
            $('.success-message').html('File is Processing');
            $('.success-message').show();
            $.ajax({
                url: ajax_object.ajax_url,
    ...
    $("#wp-chatbot-data_post_converter").on('click','.qcld_delete_training_file',function(){
        ...
        $.ajax({
            ...
            dataType: "JSON",
            data: {
                action : 'qcld_openai_delete_training_file',
                nonce:ajax_object.ajax_nonce,
                file: file,
            },
            success: function(res) {
    ...
    var settingsgoogleAi = document.getElementById("qcld_palm_save_setting");
    if(settingsgoogleAi){
        $('.qcl-openai').on('click', '#qcld_palm_save_setting', function(){
            ...
            var api_key = $( "input[name='api_key']" ).val();
            var ai_engines = $("[id*='googleai_engines'] :selected").val();
            ...
            $.ajax({
                ...
                type:'POST',
                data:    ({
                    action  : 'googleai_settings_option',
                    nonce:ajax_object.ajax_nonce,
                    api_key: api_key,
