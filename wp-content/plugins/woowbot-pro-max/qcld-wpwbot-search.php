...
function qcld_wpbo_search_site() {
	...
	if( $enable_post_types && is_array( $enable_post_types ) && ! empty( $enable_post_types ) ){
		$posttypes = implode("','",$enable_post_types);
	}
	
	//advance query building
	if( get_option('active_advance_query') != '1'){
	
		$sql = "SELECT * FROM ". $wpdb->prefix."posts where post_type in ('$posttypes') and post_status='publish' and ((post_title LIKE '%".$searchkeyword."%')) order by ID DESC";
		$limit = " Limit 0, ".$searchlimit;

	}else{
		//advance query building
		$sql = "SELECT * FROM ". $wpdb->prefix."posts where post_type in ('$posttypes') and post_status='publish' and ((post_title REGEXP '\\b".$searchkeyword."\\b') or (post_content REGEXP '\\b".$searchkeyword."\\b')) order by ID DESC";
		$limit = " Limit 0, ".$searchlimit;
	}
	...
	if ( !empty( $results ) ) {
		...
		foreach ( $results as $result ) {
				...
				$url_lang_check = $urlss ? $urlss[$selected_lan] : '' ;
				if( in_array($selected_lan, $languages, true) && (count($languages) >= 2) ){
					if( strpos(get_permalink($result->ID), $urlss[$selected_lan]) !== false){
						...
						if($result->post_type=='product'){
							if ( class_exists( 'WooCommerce' ) ) {
								...
								$responses .=			'<p class="wpbot_product_price">'.$product->get_price_html().'</p>';
							}
						}
						$responses .=		'</div>';
						$responses .=	'</a></div>';
		...
		if(  !empty( $results) ){
			...
			foreach ($results as $value) {
				//
				$selected_lan = sanitize_text_field($_POST['language']);
				$featured_img_url = get_the_post_thumbnail_url( $value[0]->ID,$imagesize);
				...
				$url_lang_check = $urlss ? $urlss[$selected_lan] : '' ;
				if( in_array($selected_lan, $languages, true) && (count($languages) >= 2) ){
					if( strpos(get_permalink($value[0]->ID), $url_lang_check) !== false){

						$total_post = $total_post + 1;
						$responses .='<div class="wpbot_card_wraper">';
						$responses .=	'<div class="wpbot_card_image '.($featured_img_url==''?'wpbot_card_image_saas':'').'"><a href="'.get_permalink( $value[0]->ID).'" '.($new_window==1?'target="_blank"':'').'>';
						$featured_img_url = get_the_post_thumbnail_url($value[0]->guid,$imagesize);
...
function qc_wpbo_search_response(){
	...
	$str_max_limit = !empty(get_option('qc_bot_str_max_limit')) ? get_option('qc_bot_str_max_limit') : '3'; 
	...
	if(empty($response_result) && class_exists('Qcld_str_pro')){
		...
		if(in_array($keyword, $categories)){
			$field ="name";
			$get_category =  $wpdb->get_row($wpdb->prepare("SELECT * FROM %i WHERE 1 and %i = %s", $cattable,$field, $keyword));
			if ( ! empty( $get_category ) ) {
				$field ="custom";
				$get_sub_cats = $wpdb->get_results($wpdb->prepare("SELECT * FROM %i  WHERE 1 and %i = %d",$cattable,$field,$get_category->id));
				if ( ! empty( $get_sub_cats ) ) {
					foreach ( $get_sub_cats as $sub_cat ) {
						$response_result[] = array('id'=>$result->id,'query'=>$sub_cat->name, 'response'=>'', 'followup'=>'', 'trigger_intent' => '', 'score'=>1);
					}
				}
			}
	...
	if(empty($response_result)){
		$field = 'query';
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM %i WHERE 1 and %i = %s and `lang`=%s  and `hidden` = 0 limit %d",$table,$field,$keyword,$language,$str_max_limit));
	...
	if( empty( $response_result ) ){
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM %i WHERE 1 and ( CONCAT(',', keyword, ',') like  %s ) or (CONCAT(',', keyword, ',') like %s) or (CONCAT(',', keyword, ',') like %s) or ( CONCAT(',', keyword, ',') like %s) or ( CONCAT(',', keyword, ',') like  %s )  and (lang = %s and (`hidden` = 0)) limit %d",$table,$keyword, $keyword ,$keyword, $keyword, $keyword ,$language,$str_max_limit));
 ...
 // ****************** for bracket problem ******************//
	if( empty( $response_result ) ){
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$table` WHERE 1 and keyword like  %s  or  response like %s or query like %s and lang = %s and `hidden` = 0 limit %d",'% '.$keyword.' %','% '.$keyword.' %','% '.$keyword.' %',$language,$str_max_limit));
		if(!empty($results)){
			foreach($results as $result){
	...
	if( empty( $response_result ) ){
		$results =$wpdb->get_results($wpdb->prepare("SELECT * FROM %i WHERE `query` REGEXP %s and lang=%s and `hidden` = 0 limit %d",$table,preg_quote($keyword),$language,$str_max_limit));
	...
	if(empty($response_result)){
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT `id`, `query`, `response`, `custom`, `trigger_intent`, MATCH($qfields) AGAINST( %s IN NATURAL LANGUAGE MODE) as score FROM %i WHERE MATCH($qfields)  AGAINST(%s IN NATURAL LANGUAGE MODE) and lang=%s and `hidden` = 0 order by score desc limit %d",'%'.$keyword.'%',$table,'%'.$keyword.'%',$language,$str_max_limit));
	...
	if(empty($response_result)){
		$field = "intent";
		$sql_intent = $wpdb->prepare("SELECT * FROM %i WHERE %i = %s",$table,$field,$keyword);
		$results = $wpdb->get_results($sql_intent);
