...
class Ayyash_Addons_Admin_Field_Select extends Ayyash_Addons_Admin_Field_Base {
	...
	public function render() {
		...
		if ( isset( $this->field['options'] ) ) {
			...
			if ( ! empty( $args['ajax'] ) ) {
				...
				$args['settings']['data']['nonce'] = wp_create_nonce( 'ayyash_addons_chosen_ajax_nonce' );
			...
			if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
				...
				if ( ! empty( $this->field['empty_message'] ) ) {
					...
					esc_html_e( 'No data available.', 'ayyash-addons' );
				}
			}
}
