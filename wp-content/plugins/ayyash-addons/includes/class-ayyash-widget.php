 ...
 * @package AyyashAddons
 */
class Ayyash_Widget extends Widget_Base {
	...
	public function is_preview_mode( $post_id = null ) {
		return \Elementor\Plugin::$instance->preview->is_preview_mode( $post_id ) || \Elementor\Plugin::$instance->editor->is_edit_mode( $post_id );
	}
	...
	protected function get_base_id() {
		if ( null === $this->base_id ) {
			$this->base_id = str_replace( 'ayyash-', '', $this->get_name() );
		}
	...
	public function get_custom_help_url() {
		return 'https://wpayyash.com/docs/ayyash-addons/widgets' . $this->get_base_id();
	}
	...
	protected function add_render_attributes() {
		...
		if ( apply_filters( "ayyash_addons/widgets/{$this->get_base_id()}/use_style_type", $this->use_style_type ) ) {
			$settings = $this->get_settings();

			$widget_type = $this->get_base_id() . 'includes' . ( ! empty( $settings['_skin'] ) ? $settings['_skin'] : 'default' );
			$settings    = $this->get_settings_for_display();
			$style_id    = $this->get_style_id();
			if ( isset( $settings[ $style_id ] ) ) {
				$widget_type = $this->get_base_id() . 'includes' . ( ( 'one' === $settings[ $style_id ] || empty( $settings[ $style_id ] ) ) ? 'default' : $settings[ $style_id ] );
			}
		...
		foreach ( $dependencies as $dependency ) {
			...
			foreach ( $dependency['plugins'] as $plugin ) {
				...
				/* translators: 1. Required Plugin Name. 2. Plugin installation/activation URL */
					__( '<strong>%1$s</strong> is not installed/activated on your site. Click <a href="%2$s" target="_blank" rel="noopener noreferrer">here</a> to install/activate <strong>%1$s</strong> first.', 'ayyash-addons' ),
					$plugin['name'],
					esc_url( ayyash_addons_plugin_install_url( $plugin['slug'] ) )
				);
			}
			...
			if ( ! empty( $messages ) ) {
				...
				$control_id = 'ayyash_addons_dependency_alert_' . $this->get_style_id() . '_' . $control_id;
	...
	protected function get_style_slug( $settings ) {
		...
		$slug = isset( $settings[ 'ayyash_addons_' . $key ] ) ? $settings[ 'ayyash_addons_' . $key ] : '';
		if ( ! $slug ) {
	...
	protected function render_icon( $key, $default = '', $settings = [], $tag = 'i', $compatibility_key = 'ayyash-addons' ) {
		if ( empty( $settings ) ) {
			$settings = $this->get_settings_for_display();
		}
		...
		if ( $has_icon ) {
			if ( $is_new || $migrated ) {
				...
				if ( $default ) {
					?>
					<<?php ayyash_addons_tag_name( $tag ); ?> class="<?php echo esc_attr( $default ); ?>" aria-hidden="true"></<?php ayyash_addons_tag_name( $tag ); ?>>
					<?php
				}
			}
		}
	...
	private function is_plugin_file( $file_path ) {
		return (
			0 === strpos( $file_path, untrailingslashit( AYYASH_ADDONS_PATH ) )
		);
	}
	 ...
	 * @return array
	 */
	protected function list_templates( $slug, $path = AYYASH_ADDONS_WIDGETS_PATH ) {
		return [
	...
	protected function list_controller( $slug = '', $path = AYYASH_ADDONS_WIDGETS_PATH ) {
		$controller = [
			sprintf(
	 ...
	 * @return array
	 */
	protected function list_content_templates( $slug, $path = AYYASH_ADDONS_WIDGETS_PATH ) {
		return [
			// @XXX if we introduce template override from the theme, then it would be added on the top of main template.
			sprintf(
