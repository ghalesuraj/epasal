...
class Client {
	...
	protected $service_endpoints = [
		'tracker' => 'https://go.wpayyash.com/api',
		'license' => 'https://apitesting.test/?wc-api=wc-am-api'
	];
	...
	public function __construct( $hash, $name, $file, $product_id = null, $args = [] ) {
		...
		if ( ! is_string( $file ) ) {
			...
			/* translators: 1. Argument Type. */
				esc_html__( 'The \'$file\' argument expected be a valid file path string. Got %s.', 'ayyash-addons' ),
				gettype( $file )
			);
		...
		if ( ! file_exists( $file ) || ! is_file( $file ) ) {
			...
			/* translators: 1. Current Class Name. */
				esc_html__( 'Invalid Argument. The \'$file\' argument needs to be a valid file path for initializing "%s" class', 'ayyash-addons' ),
				__CLASS__
			);
			_doing_it_wrong( __METHOD__, $message, '1.0.0' );
	...
	public function updater() {
		...
		if ( is_null( $this->license ) ) {
			...
			/* translators: 1. Class Method Name. */
				esc_html__( 'Updater needs License instance to be created before it. Please call "%s" first.', 'ayyash-addons' ),
				__CLASS__ . '::license'
			);
			_doing_it_wrong( __METHOD__, $message, '1.0.0' );
	...
	private function endpoint( $route = '', $server = '' ) {
		 ...
		 * @param array $params
		 */
		$route = apply_filters( 'ayyash_addons_service_api_' . $this->slug . '_request_route', $route );
		 ...
		 */
		$endpoint = apply_filters( 'ayyash_addons_service_api_' . $this->slug . '_request_endpoint', $endpoint, $route, $this->apiVersion, $this->clientVersion );
	...
	public function is_debug() {
		return apply_filters( 'ayyash_addons_service_api_is_debugging', $this->is_debug );
	}
	...
	public function is_local_request() {
		...
		return apply_filters( 'ayyash_addons_service_api_is_local', in_array( $_SERVER['REMOTE_ADDR'], [
			'127.0.0.1',
			'::1'
		] ) ); // phpcs:ignore
	}
	...
	public function request( $args = [] ) {
		...
		if ( $args['route'] || 'license' === $args['server'] ) {
			...
			return new WP_Error( 'url-route-missing', __( 'No Route or URL was set for the request.', 'ayyash-addons-services' ) );
		}
