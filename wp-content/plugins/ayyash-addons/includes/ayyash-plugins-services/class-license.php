...
class License {
	...
	public function __construct( Client $client ) {
		$this->client        = $client;
		$this->hash          = md5( $this->client->getSlug() . $this->client->getProductId() . home_url() );
		$this->option_key    = 'ayyash_addons_service_api_' . $this->hash . '_manage_license';
		...
		/* translators: %s: Plugin Name */
			sprintf( esc_html__( 'Active %s license to get professional support and automatic update from your WordPress dashboard.', 'ayyash-addons' ), '<strong>' . esc_html( $this->client->getName() ) . '</strong>' )
		);
	}
	...
	public function init() {
		...
		add_action( 'wp_ajax_ayyash_addons_dismiss_license_' . $this->hash, [ $this, '__dismiss_license' ] );
				...
				<div class="notice notice-error">
					<p><?php
						...
						/* translators: 1: Warning in bold tag, 2: This plugin name, 3: API Host Name, 4: WP_ACCESSIBLE_HOSTS constant */
							esc_html__( '%1$s You\'re blocking external requests which means you won\'t be able to get %2$s updates. Please add %3$s to %4$s constant.', 'ayyash-addons' ),
							'<b>' . esc_html__( 'Warning!', 'ayyash-addons' ) . '</b>',
	...
	protected function inactive_license_notice() {
		if ( 'no' === get_user_option( 'aa-license-notice' . $this->hash ) ) {
			return;
		}
		...
		?>
		<div class="notice notice-error is-dismissible" id="<?php echo esc_attr( $this->hash ); ?>">
			<p><?php
				printf(
				/* translators: 1: This plugin name, 2: Activation Page URL, 3: This Plugin Name */
					esc_html__( 'The %1$s API Key has not been activated, so some features are inactive! %2$s to activate %3$s.', 'ayyash-addons' ),
					'<strong>' . esc_attr( $this->client->getName() ) . '</strong>',
					esc_attr( $this->client->getType() ),
					'<a href="' . esc_url( $this->get_page_url() ) . '">' . esc_html__( 'Click here', 'ayyash-addons' ) . '</a>',
					'<strong>' . esc_attr( $this->client->getName() ) . '</strong>'
				);
				?></p>
		</div>
		<script>
			(function ($) {
				$('body').on('click', '#<?php echo esc_attr( $this->hash ); ?> .notice-dismiss', function (e) {
					e.preventDefault();
					wp.ajax.post('ayyash_addons_dismiss_license_<?php echo esc_attr( $this->hash ); ?>', {_wpnonce: '<?php echo esc_attr( wp_create_nonce( 'ayyash-addons-dismiss-licence-notice-' . $this->hash ) ); ?>'});
				});
			})(jQuery);
		</script>
		<?php
	...
	public function __dismiss_license() {
		check_ajax_referer( 'ayyash-addons-dismiss-licence-notice-' . $this->hash );

		update_user_option( get_current_user_id(), 'aa-license-notice' . $this->hash, 'no' );
	...
	protected function request( $action, $license = [] ) {
		...
		if ( ! in_array( $action, array_keys( $actions ) ) ) {
			return [
				'success' => false,
				'error'   => __( 'Invalid Request Action.', 'ayyash-addons' ),
			];
		}
		// parse license data
		...
		if ( ! $this->validate_license_data( $license ) ) {
			return [
				'success' => false,
				'error'   => __( 'Invalid/Empty License Data.', 'ayyash-addons' ),
			];
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( empty( $response ) || ! isset( $response['success'] ) || ( isset( $response['success'] ) && ! $response['success'] ) ) {
				return [
					...
					'error'   => isset( $response['error'] ) ? $response['error'] : __( 'Unknown error occurred, Please try again.', 'ayyash-addons' ),
	...
	public function set_menu_args( $args = [] ) {
		$defaults        = [
			...
			'page_title'  => __( 'Manage License', 'ayyash-addons' ),
			'menu_title'  => __( 'Manage License', 'ayyash-addons' ),
		...
		if ( $this->did_init ) {
			_doing_it_wrong(
				...
				/* translators: 1. Class Method. */
					__( '%s Should be called before License::init()', 'ayyash-addons' ),
	...
	public function render_menu_page() {
		...
		?>
		<div class="wrap ayyash-addons-srv-license-settings-wrapper">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'License Settings', 'ayyash-addons' ); ?></h1>
	...
	public function render_license_page() {
		...
		?>
		<?php do_action( 'before_ayyash_addons_service_api_' . $this->client->getSlug() . '_license_section' ); ?>
		<div class="ayyash-addons-srv-license-settings ayyash-addons-srv-license-section">
			<?php $this->render_page_header(); ?>
			<div class="ayyash-addons-srv-license-details">
				<?php
		...
		</div> <!-- /.ayyash-addons-srv-license-settings -->
		<?php do_action( 'after_ayyash_addons_service_api_' . $this->client->getSlug() . '_license_section' ); ?>
		<?php
	}
	...
	private function licenses_style() {
		...
		<style>
			.ayyash-addons-srv-license-settings * {
				-webkit-box-sizing: border-box;
				box-sizing: border-box
			}

			.ayyash-addons-srv-license-settings button {
			...
			.ayyash-addons-srv-license-settings a,
			.ayyash-addons-srv-license-settings h1,
			.ayyash-addons-srv-license-settings h2,
			.ayyash-addons-srv-license-settings h3,
			.ayyash-addons-srv-license-settings h4,
			.ayyash-addons-srv-license-settings input,
			.ayyash-addons-srv-license-settings select,
			.ayyash-addons-srv-license-settings i,
			.ayyash-addons-srv-license-settings span:not(.dashicons),
			.ayyash-addons-srv-license-settings div,
			.ayyash-addons-srv-license-settings p {
			...
			.ayyash-addons-srv-license-section {
				width: 100%;
				min-height: 1px;
				-webkit-box-sizing: border-box;
				box-sizing: border-box
			...
			.ayyash-addons-srv-license-title {
			...
			.ayyash-addons-srv-license-title svg.default-icon {
				width: 30px;
				height: 30px;
				fill: #0082bf
			}

			.ayyash-addons-srv-license-title h2 {
				margin: 10px;
				color: #1d2327
			}

			.ayyash-addons-srv-license-details {
				padding: 20px
			}
			...
			.ayyash-addons-srv-license-fields .input-group {
				position: relative;
				-webkit-box-flex: 0;
				-ms-flex: 1 1 82%;
				flex: 1 1 82%;
			...
			.ayyash-addons-srv-license-fields .input-group input,
			.ayyash-addons-srv-license-fields .input-group select {
			...
			.ayyash-addons-srv-license-fields .input-group input[readonly],
			.ayyash-addons-srv-license-fields .input-group select[readonly] {
				cursor: default
			}

			.ayyash-addons-srv-license-fields .input-group input:focus,
			.ayyash-addons-srv-license-fields .input-group select:focus {
				outline: 0 none;
				border: 1px solid #e8e5e5;
			...
			.ayyash-addons-srv-license-fields .input-group .license-input {
				position: relative;
				display: block;
				float: left;
				padding: 0 2px
			...
			.ayyash-addons-srv-license-fields .input-group .icon-wrap {
				width: 39px;
				height: 39px;
				position: absolute;
				left: 6px;
			...
			.ayyash-addons-srv-license-fields .input-group svg {
				fill: #000000;
				width: 16px;
				height: 16px;
			}

			.ayyash-addons-srv-license-fields .input-group .license-input.code {
				width: 100%
			}

			.ayyash-addons-srv-license-fields .input-group .license-input.code.has-product-id {
				width: calc(100% - 240px)
			}

			.ayyash-addons-srv-license-fields .input-group .license-input.product-id {
				width: 225px;
				margin-left: 15px
			}

			.ayyash-addons-srv-license-fields {
				display: -webkit-box;
				display: -ms-flexbox;
				display: flex;
			...
			.ayyash-addons-srv-license-fields button {
				margin-left: 20px;
				color: #645FF5;
				font-size: 15px;
				padding: 8px;
			...
			.ayyash-addons-srv-license-fields button.deactivate-button {
				border-color: #f02e5e;
				background: transparent;
				color: #f02e5e;
			}

			.ayyash-addons-srv-license-fields button:focus {
				outline: 0 none
			}
	...
	private function render_activation_form( $action ) {
		$submit_label = 'activate' == $action ? __( 'Activate License', 'ayyash-addons' ) : __( 'Deactivate License', 'ayyash-addons' );
		...
		<form method="post" action="<?php $this->formActionUrl(); ?>" spellcheck="false" autocomplete="off">
			...
			<div class="ayyash-addons-srv-license-fields">
					...
					<?php } else {
						if ( empty( $this->product_data ) ) {
							...
							<div class="license-input product-id">
								<span class="icon-wrap" aria-hidden="true">
									<svg height="512" viewBox="0 0 512 512" width="512"
										...
										<g>
											<path d="m30 30h53.857v-30h-83.857v83.857h30z"/><path
								...
								<label for="product_id"
									   class="screen-reader-text"><?php esc_html_e( 'Product ID', 'ayyash-addons' ); ?></label>
								<input class="regular-text" id="product_id" type="text"
									   value="<?php echo esc_attr( $this->license['product_id'] ); ?>"
									   placeholder="<?php esc_attr_e( 'Product ID', 'ayyash-addons' ); ?>"
							...
							<div class="license-input product-id">
								...
								<label for="product_id"
									   class="screen-reader-text"><?php esc_html_e( 'Select License Type', 'ayyash-addons' ); ?></label>
								<input type="hidden" name="<?php echo esc_attr( $this->data_key ); ?>[product_id]"
									   value="<?php echo esc_attr( $this->client->getName() ); ?>">
								<select class="regular-text" id="product_id"
										name="<?php echo esc_attr( $this->data_key ); ?>[product_id]" <?php wp_readonly( ( 'deactivate' == $action ), true ); ?><?php disabled( ( 'deactivate' == $action ), true ); ?>>
									<option value="" disabled
											selected><?php esc_html_e( 'Select A License Type', 'ayyash-addons' ); ?></option>
									<?php
				...
				<button type="submit" name="<?php echo esc_attr( $this->data_key ); ?>[submit]"
						class="<?php echo esc_attr( $action ); ?>-button"><?php echo esc_html( $submit_label ); ?></button>
				<?php if ( $this->manage_license_url ) { ?>
					<a href="<?php echo esc_url( $this->manage_license_url ); ?>"
					   class="button button-primary button-license-manage"
					   target="_blank"><?php esc_html_e( 'Manage License', 'ayyash-addons' ); ?></a>
				<?php } ?>
			</div>
		</form>
		<?php
		...
		<div class="active-license-info">
			<div class="single-license-info-group">
				<div class="single-license-info license-status">
					<h3><?php esc_html_e( 'Status:', 'ayyash-addons' ); ?></h3>
					<p class="<?php echo esc_attr( $status ); ?>"><?php 'active' == $status ? esc_html_e( 'Active', 'ayyash-addons' ) : esc_html_e( 'Inactive', 'ayyash-addons' ); ?></p>
				</div>
				<?php if ( 'inactive' !== $status ) { ?>
					<div class="single-license-info license-checked_at">
						<h3><?php esc_html_e( 'Last Checked:', 'ayyash-addons' ); ?></h3>
						<p class="<?php echo esc_attr( $status ); ?>">
							...
							<a href="<?php echo esc_url( add_query_arg( [ $this->client->getSlug() . '-check-license' => wp_create_nonce( $this->client->getSlug() ) ], $this->get_page_url() ) ); ?>"
							   style="color: currentColor;text-decoration: none;">
								...
								<span
									class="screen-reader-text"><?php esc_html_e( 'Check License Status Now', 'ayyash-addons' ); ?></span>
							</a>
						</p>
					</div>
				<?php } ?>
			...
			<div class="single-license-info license-expires">
				<h3><?php esc_html_e( 'Expires:', 'ayyash-addons' ); ?></h3>
				<?php $this->render_license_expire_datetime(); ?>
			</div>
			<div class="single-license-info-group">
				<div
					class="single-license-info license-activation-count<?php if ( false !== $this->license['unlimited'] ) {
						echo ' license-unlimited';
					} ?>">
					<h3><?php esc_html_e( 'Activation Remaining:', 'ayyash-addons' ); ?></h3>
					<?php if ( 'active' == $status ) { ?>
						<?php if ( false !== $this->license['unlimited'] ) { ?>
							<p class="active">
								...
								$unlimited .= '</svg>';
								$unlimited .= '</i>';
								$unlimited .= '<span class="screen-reader-text">' . esc_html__( 'Unlimited', 'ayyash-addons' ) . '</span>';

								printf(
								/* translators: 1: Remaining activation, 2: Total activation (limit) */
									esc_html__( '%1$d out of %2$s', 'ayyash-addons' ),
						...
						<?php } else { ?>
							<p class="<?php echo $remaining ? 'active' : 'inactive'; ?>">
								<?php
								/* translators: 1: Remaining activation, 2: Total activation (limit) */
								printf( esc_html__( '%1$d out of %2$d', 'ayyash-addons' ), esc_attr( $remaining ), esc_attr( $this->license['limit'] ) );
								?>
							</p>
						<?php } ?>
					<?php } else { ?>
						<p class="inactive"><?php esc_html_e( 'N/A', 'ayyash-addons' ); ?></p>
					<?php } ?>
				</div>
				<div class="single-license-info">
					<h3><?php esc_html_e( 'Automatic Update:', 'ayyash-addons' ); ?></h3>
					<p class="<?php echo esc_attr( $status ); ?>"><?php 'active' === $status ? esc_html_e( 'Enabled', 'ayyash-addons' ) : esc_html_e( 'Disabled', 'ayyash-addons' ); ?></p>
				</div>
			</div>
		</div>
		<?php
	...
	protected function render_license_expire_datetime() {
		if ( 'active' !== $this->license['status'] ) {
			...
			if ( $time_diff > 0 && $time_diff < WEEK_IN_SECONDS ) {
				...
				$expires  = sprintf( __( 'In %s', 'ayyash-addons' ), human_time_diff( current_time( 'timestamp', true ), $this->license['expires'] ) );
				$expiring = true;
			} elseif ( $time_diff < 0 && absint( $time_diff ) < WEEK_IN_SECONDS ) {
				// translators: placeholder is human time diff (e.g. "3 weeks")
				$expires  = sprintf( __( '%s ago', 'ayyash-addons' ), human_time_diff( current_time( 'timestamp', true ), $this->license['expires'] ) );
				$expiring = true;
			} else {
				$zone     = wp_timezone();
	...
	private function render_page_header() {
		...
		?>
		<header class="ayyash-addons-srv-license-title">
			<?php if ( ! $this->header_icon_url ) { ?>
				<svg class="default-icon" viewBox="0 0 300 300" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
				<path
					d="m150 161.48c-8.613 0-15.598 6.982-15.598 15.598 0 5.776 3.149 10.807 7.817 13.505v17.341h15.562v-17.341c4.668-2.697 7.817-7.729 7.817-13.505 0-8.616-6.984-15.598-15.598-15.598z"/>
					<path
						d="m150 85.849c-13.111 0-23.775 10.665-23.775 23.775v25.319h47.548v-25.319c-1e-3 -13.108-10.665-23.775-23.773-23.775z"/>
					<path
						d="m150 1e-3c-82.839 0-150 67.158-150 150 0 82.837 67.156 150 150 150s150-67.161 150-150c0-82.839-67.161-150-150-150zm46.09 227.12h-92.173c-9.734 0-17.626-7.892-17.626-17.629v-56.919c0-8.491 6.007-15.582 14.003-17.25v-25.697c0-27.409 22.3-49.711 49.711-49.711 27.409 0 49.709 22.3 49.709 49.711v25.697c7.993 1.673 14 8.759 14 17.25v56.919h2e-3c0 9.736-7.892 17.629-17.626 17.629z"/>
			</svg>
			<?php } else { ?>
				<img src="<?php echo esc_url( $this->header_icon_url ); ?>"
					 alt="<?php esc_attr_e( 'License Management', 'ayyash-addons' ); ?>">
			<?php } ?>
			<h2><?php
				printf(
				/* translators: 1. Theme/Plugin Name. */
					esc_html__( '%s License Management', 'ayyash-addons' ),
					$this->client->getName()
				);
				?></h2>
		</header>
	...
	private function activate_client_license( $postData ) {
		...
		if ( empty( $postData['license_key'] ) ) {
			$this->error = __( 'The license key field is required.', 'ayyash-addons' );
			...
			if ( ! $deactivate['success'] ) {
				...
				if ( $check['success'] && isset( $check['activated'] ) && $check['activated'] ) {
					if ( $deactivate['error'] ) {
						...
						$this->error = __( 'Unknown error occurred.', 'ayyash-addons' );
					}
		...
		if ( ! $updateKey ) {
			$this->success = __( 'License activated successfully.', 'ayyash-addons' );
		} else {
			$this->success = __( 'License updated successfully.', 'ayyash-addons' );
		}
	...
	private function deactivate_client_license() {
		if ( ! isset( $this->license['key'] ) || empty( $this->license['key'] ) ) {
			$this->error = __( 'License key not found.', 'ayyash-addons' );
		} else {
		...
		$this->success = __( 'License deactivated successfully.', 'ayyash-addons' );

		if ( get_current_user_id() ) {
			delete_user_option( get_current_user_id(), 'aa-license-notice' . $this->hash );
			// @TODO delete all user-meta for notice on network/wp-cli deactivation.
		}
	}
	...
	private function activation_deactivation() {
		switch ( $this->client->getType() ) {
			case 'plugin':
				...
				register_deactivation_hook( $this->client->getBasename(), [ $this, 'uninstall' ] );
				add_action( 'activated_plugin', [ $this, 'redirect_to_license_page' ], 999 );
				break;
			case 'theme':
				add_action( 'switch_theme', [ $this, 'uninstall' ] );
				add_action( 'after_switch_theme', [ $this, 'schedule_cron_event' ] );
				add_action( 'after_switch_theme', [ $this, 'redirect_to_license_page' ], 999 );
				break;
		}
	}
	 ...
	 *
	 * @param string $base_name Plugin: base file|Theme: old theme/stylesheet name.
	 *
	 * @return void
	 */
	public function redirect_to_license_page( $base_name ) {

		$canRedirect = false;

		if ( 'plugin' == $this->client->getType() ) {
			$canRedirect = ( $base_name == $this->client->getBasename() );
		}
	 ...
	 * @return string
	 */
	private function get_input_license_key( $license ) {
		// phpcs:disable
