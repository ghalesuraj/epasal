...
class MailChimp {
	...
	public function __construct( $api_key = '', $load = false ) {
		$this->user_agent = 'WordPress/' . get_bloginfo( 'version' ) . ' (Plugin; Client ' . $this->version . ') AyyashAddons/' . AYYASH_ADDONS_VERSION;

		if ( ayyash_addons_has_pro() ) {
			$this->user_agent .= ' Pro/' . AYYASH_ADDONS_PRO_VERSION;
		}
	...
	public function subscribe( $email, $first_name = '', $last_name = '' ) {
		...
		if ( ! $this->active_list ) {
			return new WP_Error( 'invalid-list', __( 'Audience List Not Set!', 'ayyash-addons' ) );
		}

		if ( ! is_email( $email ) ) {
			return new WP_Error( 'invalid-email', __( 'Invalid Email Address!', 'ayyash-addons' ) );
		}
		...
		$audience = [
			...
			'language'         => $lang[0],
			'ip_signup'        => ayyash_addons_get_unsafe_client_ip(),
			'timestamp_signup' => gmdate( 'Y-m-d\TH:i:sO', ayyash_addons_get_wp_time() ),
			'opt_in'           => ayyash_addons_get_unsafe_client_ip(),
			'timestamp_opt'    => gmdate( 'Y-m-d\TH:i:sO', ayyash_addons_get_wp_time() ),
		];
		...
		if ( is_wp_error( $data ) ) {
			if ( 'member-exists' === $data->get_error_code() ) {
				return new WP_Error( 'member-exists', __( 'Already Subscribed', 'ayyash-addons' ), $data->get_error_data() );
			}
	...
	protected function request( $path, $data = [], $method = 'GET' ) {
		...
		if ( ! $this->has_key() ) {
			return new WP_Error( 'no-api-key', __( 'No/Invalid API Key', 'ayyash-addons' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( 200 === $response_code ) {
				...
				if ( ! $data ) {
					$data = (object) [
						'type'     => 'Unknown Error',
						'title'    => __( 'Something Went Wrong', 'ayyash-addons' ),
						'status'   => $response_code,
						'detail'   => __( 'Something Went Wrong. Please Try after sometime.', 'ayyash-addons' ),
						'instance' => '',
					];
					switch ( $response_code ) {
						case 429:
						case 403:
							$data->type   = __( 'Too Many Requests', 'ayyash-addons' );
							$data->detail = __( 'Too Many Requests. Please Try after sometime.', 'ayyash-addons' );
							break;
						case 500:
							$data->type   = __( 'Internal Server Error', 'ayyash-addons' );
							$data->detail = __( 'Something Went Wrong. Please Try after sometime.', 'ayyash-addons' );
							break;
					}
				}
