...
?>
	<div class="ayyash-addons-settings-panel widgets">
		<div class="ayyash-addons-settings-panel__header">
			<div class="widget-left">
				<h3><?php esc_html_e('Ayyash Addons Widgets', 'ayyash-addons'); ?></h3>
			</div>
			<div class="widget-right">
				<span><?php esc_html_e('Global Control:', 'ayyash-addons'); ?></span>
				<div class="btn-group">
					<button type="button" class="btn btn-green toggle-widget all-on"><?php esc_html_e('Enable All', 'ayyash-addons'); ?></button>
					<button type="button" class="btn btn-red toggle-widget all-off"><?php esc_html_e('Disable All', 'ayyash-addons'); ?></button>
				</div>
				<span class="info-text"><?php esc_html_e('Active or deactivate all widgets at once.', 'ayyash-addons'); ?></span>
			</div>
		</div>
		<div class="ayyash-addons-settings-panel__body">
			<ul class="widget-filter">
				<li><a href="#" class="active" data-filter="all"><?php esc_html_e('All', 'ayyash-addons'); ?></a></li>
				<li><a href="#" data-filter="free"><?php esc_html_e('Free', 'ayyash-addons'); ?></a></li>
				<li><a href="#" data-filter="pro"><?php esc_html_e('Pro', 'ayyash-addons'); ?></a></li>
<!--				<li><a href="#" data-filter="upcoming">--><?php //esc_html_e('Upcoming', 'ayyash-addons'); ?><!--</a></li>-->
			</ul>
			<form action="#" class="ayyash-addons-widgets-form" id="ayyash-addons-widgets-settings" method="post">
				<div class="widget-container">
					<h4><?php esc_html_e('Content Widgets', 'ayyash-addons'); ?></h4>
					<div class="row">
						<?php

						$widgets_option = Plugin::instance()->get_widgets_settings();

						foreach ( Plugin::get_widgets() as $key => $val ) {
							...
							if ( $val['is_pro'] && ! ayyash_addons_has_pro() ) {
								$is_active = false;
								$disabled  = true;
							}
						...
						?>
							<div class="widget-item <?php echo $val['is_pro'] ? 'is-pro' : 'is-free'; ?> <?php echo $val['is_upcoming'] ? 'is-upcoming' : ''; ?>">
								<div class="widget-content">
									<div class="type-tag">
										<?php if ( $val['is_pro'] ) { ?>
											<span class="ayyash-addons-pro"><?php esc_html_e( 'Pro', 'ayyash-addons' ); ?></span>
										<?php }?>
									</div>
									<div class="widget-item-title">
										<label for="widget-<?php echo esc_attr( $key ); ?>">
											<h4><?php ayyash_addons_render_title( $val['label'] ); ?></h4>
										</label>
									</div>
								...
								<div class="widget-footer">
									...
									<div class="type-badge">
										<?php if ( $val['is_new'] ) { ?>
											<span class="ayyash-addons-new"><?php esc_html_e( 'New', 'ayyash-addons' ); ?></span>
										<?php }?>
										<?php if ( $val['is_upcoming'] ) { ?>
											<span class="ayyash-addons-upcoming"><?php esc_html_e( 'Upcoming', 'ayyash-addons' ); ?></span>
										<?php	}?>
									</div>
								</div>
							</div>
