<?php
/**
 * Initialize AyyashAddonsPlugins Services
 *
 * @version 1.0.0
...
class Updater {
	...
	public function __construct( Client $client, License $license ) {
		...
		$this->cache_key = 'ayyash_addons_service_api_' . md5( $this->client->getSlug() ) . '_version_info';
	}
	...
	private function get_cached_version_info( $which ) {
		...
		return $this->children_to_array( $info, [ 'icons', 'banners', 'sections' ] );
	}
	...
	private function get_updates( $action ) {
		...
		if ( isset( $response['success'] ) && $response['success'] ) {
			 ...
			 * @param array $data
			 */
			$data = apply_filters( 'ayyash_addons_service_api_' . $this->client->getSlug() . '_plugin_api_info', $data, $action );

			return $this->children_to_array( (object) $data, [
				'icons',
				'banners',
				'sections',
				'compatibility',
	 ...
	 * @return array|object
	 */
	private function children_to_array( $input, $children = [] ) {
		if ( ! empty( $children ) && is_array( $children ) && ( is_object( $input ) || is_array( $input ) ) ) {
			$is_object = is_object( $input );
			foreach ( $children as $child ) {
				if ( call_user_func_array( $is_object ? 'property_exists' : 'array_key_exists', [ $input, $child ] ) ) {
					if ( $is_object ) {
