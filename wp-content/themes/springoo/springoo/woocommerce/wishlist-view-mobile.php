...
<!-- WISHLIST MOBILE -->
<ul
	class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo esc_attr( $classes ); ?>"
	...
	if ( $wishlist && $wishlist->has_items() ) :
							...
							<?php if ( $show_variation || $show_dateadded || $show_price ) : ?>
								<table class="item-details-table">
									...
									<?php if ( $show_dateadded && $item->get_date_added() ) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Added on:', 'springoo' ); ?>
											</td>
											<td class="value">
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_quantity || $show_stock_status ) : ?>
							<table class="additional-info">
								<?php if ( $show_quantity ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Quantity:', 'springoo' ); ?>
										</td>
								...
								<?php if ( $show_stock_status ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Stock:', 'springoo' ); ?>
										</td>
										<td class="value">
											<?php echo 'out-of-stock' === $item->get_stock_status() ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'springoo' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'springoo' ) . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
							...
							<div class="move-to-another-wishlist">
								...
								<?php else : ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										 ...
										 * @return string
										 */
										echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'springoo' ) ) );
										?>
									</a>
								<?php endif; ?>
							</div>
						...
						<?php if ( $show_remove_product || $repeat_remove_button ) : ?>
							<div class="product-remove">
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'springoo' ) ) ); ?>"><i class="fa fa-trash"></i></a>
							</div>
						<?php endif; ?>
					</div>
				</li>
		...
		<p class="wishlist-empty">
			 ...
			 * @return string
			 */
			echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'springoo' ) ) );
			?>
		</p>
	<?php endif; ?>
