...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the springoo_Breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public m
 * @param  array $args Arguments to pass to Springoo_Breadcrumb.
 * @return void
 */
function springoo_breadcrumb( $args = array() ) {

	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new Springoo_Breadcrumb( $args );
	}
...
class Springoo_Breadcrumb {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Springoo_Breadcrumb::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Springoo_Breadcrumb::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Springoo_Breadcrumb::set_post_taxonomy()
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			do_action( 'springoo_breadcrumb_after');
		}

		// Allow developers to filter the breadcrumb trail HTML.
		$breadcrumb = apply_filters( 'springoo_breadcrumb', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'springoo' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'springoo' ),
			'home'                => esc_html__( 'Home',                                  'springoo' ),
			'error_404'           => esc_html__( '404 Not Found',                         'springoo' ),
			'archives'            => esc_html__( 'Archives',                              'springoo' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'springoo' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'springoo' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'springoo' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'springoo' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'springoo' ),
	...
	protected function add_items() {
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				$store_listing = intval( dokan_get_option( 'store_listing', 'dokan_pages' ) );
				$this->items[] = sprintf( '<a href="%s" rel="store_list">%s</a>', esc_url( get_permalink( $store_listing ) ), esc_html( get_the_title( $store_listing ) ) );
				$this->items[] = $store_user->get_shop_name();
			}
		}
	...
	protected function add_term_archive_items() {
		...
		if ( is_paged() ) {
			$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, $term->taxonomy ) ), single_term_title( '', false ) ); // phpcs:ignore WordPressVIPMinimum.Functions.CheckReturnValue.DirectFunctionCall
		} elseif ( true === $this->args['show_title'] ) {
	...
	protected function add_post_type_archive_items() {
		...
		if ( is_paged() || is_author() ) {
			...
			if ( is_search() ) {
				$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_post_type_archive_link( 'product' ) ), get_the_title( get_option( 'woocommerce_shop_page_id' ) ) );
				$this->items[] = sprintf( $this->labels['search'], get_search_query() );
			} else {
				$this->items[] = get_the_title( get_option( 'woocommerce_shop_page_id' ) );
			}
		} elseif ( true === $this->args['show_title'] ) {
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'springoo' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'springoo' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'springoo' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'springoo' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'springoo' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'springoo' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'springoo' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'springoo' ) ) );
	...
	protected function add_search_items() {
		...
		if ( $post_page ) {
			$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( $post_page ) ), esc_html( get_the_title( $post_page ) ) );
		}
		if ( is_paged() ) {
	...
	protected function add_post_hierarchy( $post_id ) {
		...
		if ( $post_type_object->has_archive ) {
			...
			if ( class_exists( 'WooCommerce' ) && is_product() ) {
				$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type ) ), get_the_title( get_option( 'woocommerce_shop_page_id' ) ) );
			} else {
	...
	protected function add_post_terms( $post_id, $taxonomy ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name ); // phpcs:ignore WordPressVIPMinimum.Functions.CheckReturnValue.DirectFunctionCall
		}
	}
	...
	public function add_path_parents( $path ) {
		...
		$post = get_page_by_path( $path ); // phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.get_page_by_path_get_page_by_path
			...
			if ( isset( $matches ) ) {
				...
				foreach ( $matches as $match ) {
					...
					if ( isset( $match[0] ) ) {
						...
						$post = get_page_by_path( trim( $path, '/' ) ); // phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.get_page_by_path_get_page_by_path
	...
	public function add_term_parents( $term_id, $taxonomy ) {
		...
		while ( $term_id ) {
			...
			$parents[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name ); // phpcs:ignore WordPressVIPMinimum.Functions.CheckReturnValue.DirectFunctionCall
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'springoo' ) ) ) );
				} elseif ( '%monthnum%' == $tag ) {
					// If using the %monthnum% tag, add a link to the monthly archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'springoo' ) ) ) );
				} elseif ( '%day%' == $tag ) {
					// If using the %day% tag, add a link to the daily archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'springoo' ) ) ) );
				} elseif ( '%author%' == $tag ) {
