...
/**
 * Class Springoo_Customize_Multi_Color_Control
 */
class Springoo_Customize_Multi_Color_Control extends WP_Customize_Control {
	...
	public function render_content() {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			?>
			<input type="text" class="multi-color-control" data-show-opacity="<?php echo esc_attr( $show_opacity ); ?>" data-palette="<?php echo esc_attr( $palette ); ?>" data-title="<?php echo esc_attr( $label ); ?>" data-default-color="<?php echo esc_attr( $default ); ?>" value="<?php echo esc_attr( $this->settings[ $key ]->value() ); ?>" <?php echo esc_attr( $this->get_link( $key ) ); ?> />
			<?php
		}
	}
}
