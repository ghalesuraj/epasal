<div id="dokan-seller-listing-wrap" class="grid-view v2">
	<div class="seller-listing-content">
		<?php
		$column = springoo_get_mod( 'dokan_template_store_columns' );
		?>
		<ul class="dokan-seller-wrap springoo-seller-wrap-preloader mb-0">
				...
				foreach ( $sellers['users'] as $seller ) {
					...
					<li class="dokan-single-seller woocommerce coloum-<?php echo esc_attr( $per_row ); ?> <?php echo ( ! $store_banner_id ) ? 'no-banner-img' : ''; ?>">
						<div class="store-wrapper">
								...
								<div class="store-data-container">
									...
									<div class="store-data <?php echo esc_attr( $store_open_is_on ); ?>">
										<div class="store-data-header">
											...
											<button data-bs-target="#springoo_vendor_contact" data-bs-toggle="modal" title="<?php esc_attr_e( 'Inquiry', 'springoo' ); ?>"><i class="far fa-question-circle"></i> Inquiry
											</button>
										</div>
										...
										if ( ! empty( $store_rating['count'] ) ) : ?>
											...
											// translators: 1) seller rating
												esc_attr__( 'Rated %s out of 5', 'springoo' ), number_format_i18n( $store_rating['rating'] ) //phpcs:ignore  (WordPress.Security.EscapeOutput.OutputNotEscaped)
											);
											?>">
                                                ...
                                                <p class="rating">
													...
													echo esc_html(
													// translators: 1) seller rating
														sprintf( __( '%s out of 5', 'springoo' ), number_format_i18n( $store_rating['rating'] ) ) //phpcs:ignore  (WordPress.Security.EscapeOutput.OutputNotEscaped)
													);
													?>
                                                </p>
                                            </div>
							...
							<div class="store-footer">
								<a href="<?php echo esc_url( $store_url ); ?>" title="<?php esc_attr_e( 'View Store', 'springoo' ); ?>">
									<?php esc_html_e( 'View Store', 'springoo' ); ?>
								</a>
					...
					<div class="modal fade" id="springoo_vendor_contact" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="springoo_vendor_contact" aria-hidden="true">
						<div class="modal-dialog modal-dialog-centered">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="springoo_vendor_contact_title"><?php echo esc_html( 'Ask a Question' ); ?></h5>
									<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="si si-thin-close"></i></button>
								</div>
